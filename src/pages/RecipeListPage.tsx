...
const RecipeListPage = (props: Props) => {
  ...
  async function handleSubmit(e:any){ 
    try {
      e.preventDefault();
      const result = await axios.get(`https://backend-bonappetit.up.railway.app//api/v1/recipes/title/${value}`)
      setList(()=> result.data.length > 0?
      result.data:[{name:'Recipes not found'}]);
      console.log(list);
      } 
      catch (error) {
      console.log(error);
  ...
  <>
        ...
        <form className="max-w-7xl flex flex-col justify-center items-center px-20 py-5">
          <div className="flex flex-wrap gap-3">
              <input onChange={(e)=>{setTitle(e.target.value)}} className="placeholder:italic placeholder:text-slate-400 block w-fit border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Recipe name..." type="text" name="search"/>
              <input onChange={(e)=>{setType(e.target.value)}} className="placeholder:italic placeholder:text-slate-400 block w-fit border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Meal type..." type="text" name="search"/>
              <button onClick={handleSubmit} className="bg-red w-fit rounded-md py-2 pl-3 pr-3 focus:outline-none text-white sm:text-sm">Search</button>
          </div>
        </form>
